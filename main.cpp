...
void featurempap_generation_main(featuremap_data& fmpd, source_data& srcd, destination_data& dstd, analysis_results& rst)
{
    ...
    if (IS_FIRST_RUN) {
        if (fd.ft == featureType::fPCA_16bin)
            fmpd.dimFeatures = 16;
        else
    ...
    if (fm.reDetection) {
        ...
        cv::Point2d after_size = transform2d(cv::Point2d(SZ, SZ), re_before_tform);
        cv::warpPerspective(dstd.oImage, dstd.oImage, fmpd.curr_tform, cv::Size(after_size), fm.interpolation);
        if (IS_IMSHOW) {
            cv::Mat afp;
...
#if RANSAC_NORMAL 
    sc.positionestimation_normal(pe, fmpd, srcd, dstd, rst);
#elif RANSAC_grfl
    sc.positionestimation_grfl(pe, fmpd, srcd, dstd, rst);
#endif
...
void load_images(std::string imgfile, std::vector<cv::Mat>& takenImgs)
{
    ...
    for (int i = 0; i < takenImgPath.size(); i++) {
        ...
        takenImgs.push_back(cv::imread(takenImgPath[i].c_str(), cv::IMREAD_GRAYSCALE));
        if (IS_IMSHOW) {
            cv::Mat tmp;
...
int main()
{
    ...
    rst.total_image_num = takenImgs.size();

    std::chrono::system_clock::time_point st, ed;

    for (int i = 0; i < takenImgs.size() - 1; i++) {
        if (DISPLAY_CLEAR_EACH) printf("\033[2J \033[0;0H");
        else progress_log("\n");
        ...
        if (ACCURACY_EVALUATION_IN_EACH) {
            accuracy_evaluation(fmpd, srcd, dstd, rst, false);
        }
        ed = std::chrono::system_clock::now();
    ...
    save_statistics(INT_MAX, fmpd, rst);
    ...
    progress_log("地図生成の総所要時間　：%d [ms]\n", (int)rst.elapsed_time_formap_sum);
